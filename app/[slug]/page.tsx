...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    ...
    alternates: {
      canonical: `https://medtechreview.com/${article.slug}`,
    },
  }
}
...
export default function ArticlePage({ params }: Props) {
  ...
  const articleSchema = {
    ...
    author: { '@type': 'Organization', name: 'MedTech Review' },
    publisher: { '@type': 'Organization', name: 'MedTech Review' },
  }
    ...
    <>
      ...
      <article className="max-w-3xl mx-auto px-4 sm:px-6 py-10">
        ...
        <header className="mb-10">
          <span className="text-xs font-medium text-blue-700 bg-blue-50 px-2 py-0.5 rounded mb-3 inline-block">
            {article.category}
          </span>
          ...
          <div className="flex flex-wrap items-center gap-4 text-sm text-gray-400 border-t border-b border-gray-100 py-3">
            <span>
              By <strong className="text-gray-600">MedTech Review Editorial</strong>
            </span>
            <span>
              Updated:{' '}
            ...
            {article.verified && <span className="badge-verified">✓ Medically Verified</span>}
          </div>
        </header>
            ...
            {article.faqs.map((faq, i) => (
              ...
              >
                <summary className="flex items-center justify-between px-5 py-4 cursor-pointer bg-gray-50 hover:bg-gray-100 transition-colors list-none">
                  ...
                  <span className="text-gray-400 flex-shrink-0 transition-transform group-open:rotate-180">
                    ▾
                  </span>
                </summary>
                <div className="px-5 py-4 text-sm text-gray-700 leading-relaxed bg-white">
                  {faq.answer}
                </div>
              </details>
            ))}
        ...
        <section className="rounded-2xl bg-blue-950 text-white p-8 text-center">
          <p className="text-xs uppercase tracking-widest text-blue-400 mb-2">Our Top Pick</p>
          ...
          <a
            ...
            target="_blank"
            rel="nofollow noopener noreferrer"
            className="inline-block bg-white text-blue-900 font-semibold px-6 py-3 rounded-xl hover:bg-blue-50 transition-colors text-sm"
          >
